...
class LyricsSearcherApp:
    ...
    def setup_ui(self):
        ...
        tk.Button(search_frame, text="DOSYA ÜZERİNE KAYDET__HATA OLMAMASI İÇİN: \nŞarkı söz metni üzende işlem yap seç kopyala\nSoldaki şarkı İSMİ.TXT ye tıkla_altı çizili olsun\nMetni sil düzenlediğin içeriği yağıştır\nDEĞİŞİK_liği DOSYA ÜZERİNE KAYDET",command=self.emin_misin_save_file).pack(side="left",ipadx=1, ipady=1,fill="x")#bu satırı ekle buton aktif olur(,command=self.save_file).pack(side="left")
    ...
    def emin_misin_save_file(self):
        if messagebox.askyesno("Onay", "Kaydetmek istediğinize emin misiniz?\nHATA OLMAMASI İÇİN: \nSoldaki şarkı İSMİ.TXT ye tıkla_altı çizili olsun\nŞarkı söz metni üzende işlem yap\n DEĞİŞİK_liğini yap ve DOSYA ÜZERİNE KAYDET\n( Asla Favoriye TIKLAMA_MALI ve SEÇME_MELİ_SİN )"):
