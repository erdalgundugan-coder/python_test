...
class LyricsSearcherApp:
    ...
    def setup_ui(self):
        ...
        self.folder_path_label = tk.Label(folder_frame, text=f"Klasör: {self.folder_path}", font=("Arial", 9))
        ...
        tk.Button(search_frame, text="Şarkı Metni Değişikliğini Kaydet", command=self.save_file).pack(side="left")
        ...
        tk.Label(left_frame, text="Dosya Ara:", font=self.font_listbox).grid(row=3, column=0, sticky="w", pady=(5,0))
        self.file_search_entry = tk.Entry(left_frame, font=self.font_listbox)
        self.file_search_entry.grid(row=4, column=0, sticky="ew", padx=(0, 5), pady=(0, 5))
        ...
        self.btn_add_fav = tk.Button(btn_frame, text="Fav listeden Ekle", command=self.add_selected_result_to_favorites)
        ...
        self.btn_add_cursor_fav = tk.Button(btn_frame, text="Fav İmleç Satırı Ekle", command=self.add_current_line_to_favorites)
        ...
        tk.Button(fav_btn_frame, text="Boş Fav Dosya Oluştur", command=self.create_empty_favorites_file).pack(side="left", padx=5)
