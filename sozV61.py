...
class LyricsSearcherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Şarkı Sözü Arama   -D:\sozler ve videolar\sozler-\nversion V61.0 Erdal GÜNDOĞAN-20250990027")
    ...
    def setup_ui(self):
        ...
        self.file_count_label.pack(side="bottom", padx=2)
        ...
        tk.Button(search_frame, text=" ARA\n(özellik:aynı satırdaki kelime,-leri\nkelimelerin ilk heceleri baz alır )", command=self.search_in_all_files).pack(side="left", padx=(0, 10))
        tk.Button(search_frame, text="Asla Favori SEÇME_ME_LİSİN\nSoldaki şarkı İSMİ.TXT ye tıkla_altı çizili olsun\nŞarkı söz metni üzende işlem yap\n DEĞİŞİK_liğini yap ve ÜZERİNE KAYDET", command=self.save_file).pack(side="left")
        ...
        tk.Label(left_frame, text="Ara (Dosya.txt):", font=self.font_listbox,width=50).grid(row=0, column=0,sticky="w", pady=(5,0))
        self.file_search_entry = tk.Entry(left_frame, font=self.font_listbox,width=50)
        ...
        save_jpeg_button.grid(row=3, column=10, pady=5, padx=5, sticky="ne")
        ...
        self.dosya_entry = tk.Entry(right_frame, font=self.font_listbox, state="normal", readonlybackground="white",width=50)
        self.dosya_entry.grid(row=1, column=0, sticky="sen")
    ...
    def search_in_all_files(self):
        ...
        raw_input_norm = self.normalize(raw_input)  # Tam arama ifadesi normalize edilmiş
        ...
        # Favoriler de normal dosyalar gibi arama sırasına dahil edilir
        search_order = all_files

        full_match_results = []
        grouped_results = {len(keywords): [], len(keywords)-1: [], 1: []}
        ...
        for filename in search_order:
            ...
            for lineno, line in enumerate(lines):
                if lineno >= y:
                    ...
                    # Öncelikle tam arama ifadesi geçiyor mu kontrol et
                    if raw_input_norm in norm_line:
                        full_match_results.append((filename, lineno, line.strip()))
                        y = lineno + 40
                    else:
                        line_words = norm_line.split()
                        match_count = sum(any(word.startswith(k) for word in line_words) for k in keywords)

                        if match_count == len(keywords):
                            grouped_results[len(keywords)].append((filename, lineno, line.strip()))
                            y = lineno + 40
                        elif match_count == len(keywords) - 1 and (len(keywords) - 1) in grouped_results:
                            grouped_results[len(keywords) - 1].append((filename, lineno, line.strip()))
                            y = lineno + 40
                        elif match_count == 1:
                            grouped_results[1].append((filename, lineno, line.strip()))
                            y = lineno + 40

        # Öncelikle tam eşleşenler, sonra kelime sayısına göre gruplar     
        # total_results sıralama
        total_results = full_match_results
        for key in sorted(grouped_results.keys(), reverse=True):
            total_results.extend(grouped_results[key])
        fav_results = []
        nonfav_results = []

        for filename, lineno, line in total_results:
            is_fav = filename in favorite_files
            if is_fav:
                fav_results.append((filename, lineno, line))
            else:
                nonfav_results.append((filename, lineno, line))

        # Favoriler önce, diğerleri sonra
        ordered_results = fav_results + nonfav_results

        for filename, lineno, line in ordered_results:
            is_fav = filename in favorite_files
            prefix = "[FAV] " if is_fav else ""
            display_line = f"{prefix}{lineno}: {line}***[{filename}]"
            self.result_listbox.insert(tk.END, display_line)

            last_index = self.result_listbox.size() - 1  
            if is_fav:
                self.result_listbox.itemconfig(last_index, bg="yellow")
            self.search_results.append((filename, lineno, line))     
...
#.14.09.2025 hazırlan dı aramalr iyileşti,kayıt boyutu ayarlama çalışm devam