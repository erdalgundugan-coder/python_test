...
class TkinterHTTPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        # Klasördeki resim dosyaları (jpg, png, gif...)
        current_files = [
            f for f in file_list
            if os.path.isfile(os.path.join(path, f)) and f.lower().endswith((".jpg", ".jpeg", ".png", ".gif"))
        ]

        # En yeni dosya en başta olacak şekilde tarihe göre sırala
        current_files.sort(
            key=lambda a: os.path.getmtime(os.path.join(path, a)),
            reverse=True
        )
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Wi-Fi Sarki Metni Paylaşımı V1.6 - Erdal GÜNDOĞAN 2025")
