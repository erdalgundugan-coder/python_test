...
class LyricsSearcherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Kamp Keyfi Şarkı Sözü Arama  *** -D:\\sozler ve videolar\\sozler,videolar\\media_yukle,logo,qr,dosyalar  ***  version V77.0 Erdal GÜNDOĞAN-202511220001")
        ...
        # JPEG kayıt klasörü (varsayılan eski yol, ama değiştirilebilir)
        self.jpeg_save_dir = r"D,X:\sozler ve videolar\sozler"
    ...
    def setup_ui(self):
        ...
        tk.Button(folder_frame, text="Klasör\nSeç\nTXT ler", command=self.select_folder).pack(side="left")
        tk.Button(folder_frame, text="Yenile\nTXT\nDosyaları", command=self.refresh_file_list).pack(side="left", padx=5)
        
        # 🔹 JPEG kayıt klasörü seçme butonu
        tk.Button(folder_frame, text="JPEG-Sozler\nKlasör Seç\nKaydedilen", command=self.select_jpeg_folder).pack(side="left", padx=5)

        self.file_count_label = tk.Label(folder_frame, text="Dosya: 0")
        self.file_count_label.pack(side="bottom", padx=1)
        self.folder_path_label = tk.Label(folder_frame, text=f"Klasör:\n{self.folder_path}", font=("Arial", 9))
        self.folder_path_label.pack(side="left", padx=1)
        search_frame = tk.Frame(top_frame)
        search_frame.pack(side="left", padx=2) 
        #self.entry = tk.Entry(search_frame, width=50, font=self.font_textbox)
        self.entry = tk.Entry(search_frame, width=30, font=self.font_listbox)        
        ...
        tk.Button(search_frame, text=" ...ARA... Örn:(EYLÜ-l de G-el)\n EYLÜ G yaz.Özl:Kelime,-lerin\nİlk hece-den Başlar", command=self.search_in_all_files).pack(side="left", padx=(0, 1))


        self.btn_new_txt = tk.Button(search_frame, text="...FARKLI KAYDET...\nDüzenle/Yeni/Boş\nMetin Kutusu İçi", command=self.new_txt_toggle)
        self.btn_new_txt.pack(side="left",ipadx=1, ipady=1, padx=(10, 10))
        tk.Button(search_frame, text="...KAYDET...Şarkı Sözü Üzerinde\nDeğişikl Yap,SolBox Şarkı Dosyası\n Tıklanmış ve Altı_çiz Olsun",command=self.emin_misin_save_file).pack(side="left",ipadx=1, ipady=1,fill="x")
        #bu satırı ekle buton aktif olur(,command=self.save_file).pack(side="left")
        ...
        tk.Label(left_frame, text="Ara (Dosya.txt):", font=self.font_listbox,width=30).grid(row=0, column=0,sticky="w", pady=(5,0))
        self.file_search_entry = tk.Entry(left_frame, font=self.font_listbox,width=30)
        ...
        self.result_text.grid(row=0, column=0,columnspan=4, rowspan=1,sticky="nsew")
        
        scrollbar_text = tk.Scrollbar(right_frame, command=self.result_text.yview)
        scrollbar_text.grid(row=0, column=4, sticky="nsw")
        ...
        self.cursor_line_label.grid(row=1, column=1, sticky="n", pady=2)
        
        self.dosya_entry = tk.Entry(right_frame, font=self.font_listbox, state="normal", readonlybackground="white",width=35)#altta dosya adı tutuyor
        self.dosya_entry.grid(row=1, column=3, sticky="ne",columnspan=1)
        
        self.dark_button = tk.Button(right_frame, text="🌙\nDark Mode", command=self.toggle_dark_mode)
        self.dark_button.grid(row=3, column=0, pady=5, padx=5, sticky="nw")   
        copy_btn_sec = tk.Button(right_frame, text="→ Seçimi 2. Pencerede\nGöster", command=self.open_floating_copy_sec)
        copy_btn_sec.grid(row=3, column=0, pady=5, padx=5, sticky="ne")  
        self.clean_spaces_button = tk.Button(right_frame, text="Satırdaki\nBoşlukları\nTemizle", command=self.fazla_bosluklari_temizle)
        self.clean_spaces_button.grid(row=3, column=1, pady=5, padx=5, sticky="ne")  

        copy_btn = tk.Button(right_frame, text="→ Başlığı Silerek Dörtlük\n Yap,Secili-Seçimsiz Metnin\nİlk Satırını/(CTRL+f,F)", command=self.dortluk_yap_ilk_sat_sil)
        copy_btn.grid(row=3, column=2, pady=5, padx=5, sticky="nw")
        copy_btn = tk.Button(right_frame, text="→ Dörtlük Yap\nSecili-Seçimsiz Metni\n(CTRL+d,D)", command=self.dortluk_yap)
        copy_btn.grid(row=3, column=3, pady=5, padx=5, sticky="nw")
        save_jpeg_button = tk.Button(right_frame, text="JPEG Gönder\nSeçili-Seçimsiz Metni\n(CTRL+g,G)", command= self.save_noselected_text_as_jpeg)#(ctrl+j)
        save_jpeg_button.grid(row=3, column=3, pady=5, padx=5, sticky="ne")        
    ...
    def setup_bindings(self):
        ...
        self.root.bind("<Control-j>", self.save_selected_text_as_jpeg)     
        self.root.bind("<Control-g>", self.save_noselected_text_as_jpeg)  
        self.root.bind("<Control-d>", lambda event: self.dortluk_yap())
        self.root.bind("<Control-f>", lambda event: self.dortluk_yap_ilk_sat_sil())
        self.root.bind("<Control-G>", self.save_noselected_text_as_jpeg)  
        self.root.bind("<Control-D>", lambda event: self.dortluk_yap())
        self.root.bind("<Control-F>", lambda event: self.dortluk_yap_ilk_sat_sil())


    def ilk_satiri_sil(self):
        first_line = self.result_text.get("1.0", "2.0")  # 1. satırı al
        if not first_line.strip():
            return  # boşsa dokunma
        self.result_text.delete("1.0", "2.0")  # ilk satırı sil
    ...
    def load_files(self):
        ...
        self.file_count_label.config(text=f"Dosya \nSayısı:{len(self.file_contents)}")
    def update_file_list(self):      
        ...
        for filename in sorted(self.file_contents.keys(), key=lambda x: self.normalize(x)):
            self.file_listbox.insert(tk.END, filename)
    def normalize(self, text):
        text = text.casefold()
        text = unicodedata.normalize("NFKD", text)
        text = "".join(c for c in text if not unicodedata.combining(c))
        text = text.lower().replace("i̇", "i")  # noktalı İ bozulmasın
        text = text.replace("ı", "i").replace("ş", "s").replace("ç", "c").replace("ğ", "g").replace("ü", "u").replace("ö", "o")

        return text
    def filter_file_list(self, event=None):
        query = self.normalize(self.file_search_entry.get().strip())
        self.file_listbox.delete(0, tk.END)        
        for filename in sorted(self.file_contents.keys(), key=lambda x: self.normalize(x)):
            if query in self.normalize(filename):
                self.file_listbox.insert(tk.END, filename)
    ...
    def search_in_all_files(self):
        ...
        for filename in search_order:
            ...
            # --- DOSYA ADI ARAMASI ---
            if all(k in norm_filename for k in keywords):
                is_fav = filename in favorite_files
                prefix = "[FAV] " if is_fav else ""
                display_line = f"{prefix}:DOSYA_:{filename}:***{filename}"
                self.result_listbox.insert(tk.END, display_line)
                self.search_results.append((filename, -1, f"[DOSYA_:] {filename}"))
    ...
    def highlight_phrase(self, phrase):
        """Aranan öbeği Text widget içinde sarı renkle vurgula"""
        self.result_text.tag_remove("highlight_phrase", "1.0", tk.END)
        if not phrase:
            return
        start = "1.0"
        while True:
            start = self.result_text.search(phrase, start, nocase=1, stopindex=tk.END)
            if not start:
                break
            end = f"{start}+{len(phrase)}c"
            self.result_text.tag_add("highlight_phrase", start, end)
            start = end
        self.result_text.tag_config("highlight_phrase", background="yellow", foreground="black")

    def highlight_keywords(self, keywords, phrase):
        """Kelimeleri ayrı ayrı maviyle vurgula (öbekten farklı)"""
        self.result_text.tag_remove("highlight_keywords", "1.0", tk.END)
        for kw in keywords:
            if kw == phrase:  # zaten highlight_phrase boyadı
                continue
            start = "1.0"
            while True:
                start = self.result_text.search(kw, start, nocase=1, stopindex=tk.END)
                if not start:
                    break
                end = f"{start}+{len(kw)}c"
                self.result_text.tag_add("highlight_keywords", start, end)
                start = end
        self.result_text.tag_config("highlight_keywords", background="lightblue", foreground="black")
    ...
    def highlight_text_in_result(self, filename, highlight_line_no=None):
        ...
        #self.dosya_label.config(text=c)  # Dosya adını göster
        self.show_filename(os.path.basename(filename))  # Dosya adını göster
    def emin_misin_save_file(self):        
        tk.messagebox.showwarning("Kaydedilecek dosya :" ,self.current_selection)
        if messagebox.askyesno(f"Kaydedilecek dosya : {self.current_selection}\n  Onay", f"Seçili Dosya:...{self.current_selection}...."
            "\n \nÜzerine kaydedilecek.\nKaydetmek istediğinize emin misiniz?\n\n(Hata Varsa Soldan Şarkı_ismi Dosyası Seçiniz.)"):
            self.save_file()  # <-- parantez ekledik
            #print("Değişiklik kaydedildi!")
            #messagebox.showinfo("Bilgi", "Değişiklik kaydedildi!")  # <-- doğru kullanım
        else:
            #print("İşlem iptal edildi.")
            #messagebox.showinfo("Bilgi", "İptal edildi.")  # <-- doğru kullanım
            return
        
    def save_file(self):
        #KAYTI ETMEK İÇİN EMİN MİSİN
    ...
    def add_selected_result_to_favorites(self):
        ...
        if not sel:
            messagebox.showwarning("Uyarı","Seçili dosya yok...\n.. Arama sonucundan bir dosya.txt seçiniz.")
            #messagebox.showinfo("Uyarı","Seçili dosya yok...\n.. Arama sonucundan bir dosya.txt seçiniz.")
            return
        idx = sel[0]
        ...
        if (filename ==""):
            messagebox.showwarning("Uyarı","Seçili dosya yok...\n.. Arama sonucundan bir dosya.txt seçiniz.")
            return
        yanit2 = messagebox.askyesnocancel("Çıkış", f"..Arama sonucu listesindeki seçili dosya:\n \n....{filename}....\n \n \nBu şekilde şarkıyı Favori listesine eklemek ister misiniz?\n..Hatalıysa Arama sonucu bulunan şarkılardan seçerek devam edin. ")
        if yanit2 is None:
            return  # Cancel,  çıkış, iptal tuşuna basıldı,
        elif yanit2:   
            #evet tuşuna basıldı,     
            # Mevcut gruplar
    ...
    def add_current_line_to_favorites(self):
        ...
        if (filename ==""):
            messagebox.showwarning("Uyarı","seçili şarkı adı dosyası yok...\n..soldan şarkı adı dosyası seçiniz.")
            return
        yanit2 = messagebox.askyesnocancel("Çıkış", f"Şarkı sözü metni içinde seçili satır a ait şarkı adı dosyası:\n \n....{filename}....dır.\n \n \nBu şekilde Favori listesine eklemek ister misiniz?\n(..Hatalıysa sol listeden şarkı dosyasına tıklayıp, şarkı sözü satırı seçiniz..) ")
        if yanit2 is None:
            return  # Cancel,  çıkış, iptal tuşuna basıldı,
        elif yanit2:   
        #evet tuşuna basıldı,     
    ...
    def dortluk_yap(self):
        ...
        # Satır satır temizle (strip + çoklu boşluk sadeleştir)
        lines = content.splitlines()
        cleaned_lines = [re.sub(r"[ \t]+", " ", line.strip()) for line in lines if line.strip()]

        # 4 satırlık gruplar oluştur
        grouped_lines = []
        for i in range(0, len(cleaned_lines), 4):
            block = cleaned_lines[i:i+4]
            grouped_lines.append("\n".join(block))

        cleaned = "\n\n".join(grouped_lines)  # dörtlükler arası boşluk bırak

        # Seçilen metni silip yerine düzenlenmişini yaz
        if self.result_text.tag_ranges(tk.SEL):
            self.result_text.delete(tk.SEL_FIRST, tk.SEL_LAST)
            self.result_text.insert(tk.INSERT, cleaned)
        else:
            self.result_text.delete("1.0", "end")
            self.result_text.insert("1.0", cleaned)
    def dortluk_yap_ilk_sat_sil(self):
    ...
    # Dosya adı gösterme fonksiyonunda:
    def show_filename(self, filename):
        self.dosya_entry.config(state="normal")
        self.dosya_entry.delete(0, tk.END)
        self.dosya_entry.insert(0, os.path.basename(filename))
        self.dosya_entry.config(state="readonly")  
    ...
    def save_selected_text_as_jpeg(self, event=None):
        
        # Seçilen JPEG klasörü, yoksa default'u kullan
        save_dir = getattr(self, "jpeg_save_dir", r"D:\sozler ve videolar\sozler")
        ...
        filename_base = self.clean_filename(first_line)
        ...
        # En uzun satırı bul ve piksel genişliğini ölç
        dummy_img = Image.new("RGB", (1, 1))
        draw_dummy = ImageDraw.Draw(dummy_img)
        max_line = max(lines, key=lambda line: draw_dummy.textlength(line, font=font))
        max_line_width = draw_dummy.textlength(max_line, font=font)

        # Kenarlık boşluğu
        padding_x = 20
        padding_y = 20

        # JPEG boyutlarını en uzun satıra göre ayarla
        width = int(max_line_width + padding_x * 2)
        ...
        img.save(filepath, "JPEG", quality=95, subsampling=0, optimize=True)
        print(f"JPEG kaydedildi: {filepath}")
        self.show_file_in_explorer(filepath)
        self.entry.focus_set()
        self.entry.delete(0, tk.END)


    def save_noselected_text_as_jpeg(self, event=None):    
        from datetime import datetime
        from PIL import Image, ImageDraw, ImageFont
        ...
        # ✅ Seçim varsa onu al, yoksa tüm metni al
        try:
            selected_text = self.result_text.get(tk.SEL_FIRST, tk.SEL_LAST).strip()
        except tk.TclError:
            selected_text = self.result_text.get("1.0", tk.END).strip()

        if not selected_text:
            print("Metin boş, JPEG oluşturulmadı!")
            return

        # Dosya ismini ilk satırdan oluştur
        """
        first_line = selected_text.split("\n")[0].strip()
        filename_base = self.clean_filename(first_line) if hasattr(self, "clean_filename") else first_line
        if not filename_base:
            filename_base = "sozler"
        """
        first_line = selected_text.split("\n")[0]
        ...
        # Çok uzun adları kes (örneğin 50 karakterle sınırla)
        ...
        if hasattr(self, "show_file_in_explorer"):
            messagebox.showinfo(" Şarkı Sözü Telefonlara Gönderildi.",f"\n✅ Erdal GÜNDOĞAN'a teşekkür. \n\nJPEG kaydedildi:\n{filepath}")
            #self.show_file_in_explorer(filepath)
    ...
    def select_jpeg_folder(self):
        """JPEG olarak kaydedilecek resimlerin klasörünü seç"""
        folder = filedialog.askdirectory(initialdir=self.jpeg_save_dir if hasattr(self, "jpeg_save_dir") else APP_DIR)
        if not folder:
            return
        self.jpeg_save_dir = folder
        messagebox.showinfo("Bilgi", f"JPEG kayıt klasörü:\n{self.jpeg_save_dir}")

    def show_file_in_explorer(self,filepath):
        # Windows için dosyayı Explorer'da seçili gösterir
        if os.name == 'nt':
            # explorer.exe /select, "dosya tam yolu"
            subprocess.run(f'explorer /select,"{filepath}"')
        else:
            print("Bu özellik sadece Windows için geçerlidir.")
    def on_closing(self):
...
# arama İ i I ı gibi sorun düzeldi.,
#txt dosya adı kopyala ve bitti........son
#ÇOK İYİ OLDU
#47 DEN DEVAM ETTİM, ARAMALAR İYİLEŞTİRİLDİ.
#C\sarki_jpeg oluşturuldu, metinleri jpeg e çeviren buton eklendi...
#aramada hata var eşleşmeyen dosyalar var!!!!!!!!!
#.14.09.2025 hazırlan dı aramalr iyileşti,kayıt boyutu ayarlama çalışm devam-524
#self.fazla_bosluklari_temizle()#74 icin eklendi-1111 02112025