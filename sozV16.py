...
class LyricsSearcherApp:
    ...
    def setup_ui(self):
        ...
        # Ortadaki paned: Üst: Arama sonuçları, Alt: Favoriler (gruplu)
        ...
        fav_label = tk.Label(center_frame_bottom, text="Favoriler (Gruplu):", font=self.font_listbox)
        fav_label.grid(row=0, column=0, sticky="w")

        # Favoriler için Treeview (gruplama ile)
        self.fav_tree = ttk.Treeview(center_frame_bottom)
        self.fav_tree.grid(row=1, column=0, sticky="nsew")
        scrollbar_fav = tk.Scrollbar(center_frame_bottom, command=self.fav_tree.yview)
        scrollbar_fav.grid(row=1, column=1, sticky="ns")
        self.fav_tree.config(yscrollcommand=scrollbar_fav.set)
        self.fav_tree.bind("<<TreeviewSelect>>", self.show_favorite_content)
    ...
    def save_file(self):
        ...
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(content)
            self.file_contents[self.current_selection] = content.splitlines(keepends=True)
            tk.messagebox.showinfo("Başarılı", f"{self.current_selection} kaydedildi.")
        except Exception as e:
            tk.messagebox.showerror("Hata", f"Dosya kaydedilemedi: {e}")

    # Favorilerle ilgili fonksiyonlar
    def load_favorites(self):
        if os.path.exists(FAVORITES_FILE):
            try:
                with open(FAVORITES_FILE, "r", encoding="utf-8") as f:
                    favs = json.load(f)
                # Sadece doğru formatta olanları al
                self.favorites = [f for f in favs if isinstance(f, dict) and "file" in f and "line" in f and "text" in f]
            except Exception:
                self.favorites = []
        else:
            self.favorites = []
    ...
    def save_favorites(self):
        try:
            ...
            messagebox.showinfo("Başarılı", "Favoriler kaydedildi.")
        except Exception as e:
    ...
    def update_fav_list(self):
        ...
        for fav in self.favorites:
            group = fav.get("group", "Genel")
            if group not in groups:
                groups[group] = []
            groups[group].append(fav)

        # Grupları ve altındaki favorileri ekle
        for group, favs in sorted(groups.items()):
            group_id = self.fav_tree.insert("", "end", text=group, open=True)
            for fav in favs:
                display_text = f":{fav['line']}:{fav['text']} ({fav['file']})"
                self.fav_tree.insert(group_id, "end", text=display_text, values=(fav["file"], fav["line"]))
    ...
    def add_selected_result_to_favorites(self):
        ...
        # Kullanıcıdan grup adı sor
        group = askstring("Favori Grubu", "Favori için grup adı girin:", initialvalue="Genel")
    ...
    def remove_selected_favorite(self):
        ...
        if not sel:
            return
        item_id = sel[0]
        parent = self.fav_tree.parent(item_id)

        # Eğer grup ise hiçbir şey yapma
        if parent == "":
            messagebox.showwarning("Uyarı", "Bir grup seçtiniz. Favori silmek için favori öğesini seçin.")
            return

        # Favori öğe ise sil
        file_, line = self.fav_tree.item(item_id, "values")
        line = int(line)

        # Favoriler listesinden çıkar
        for i, fav in enumerate(self.favorites):
            if fav["file"] == file_ and fav["line"] == line:
    ...
    def show_favorite_content(self, event=None):
        ...
        # Eğer grup seçiliyse içerik gösterme
        if parent == "":
            return

        file_ = self.fav_tree.item(item_id, "values")[0]
        line = int(self.fav_tree.item(item_id, "values")[1])

        try:
            idx_file = self.file_listbox.get(0, tk.END).index(file_)
            self.file_listbox.selection_clear(0, tk.END)
        ...
        self.highlight_text_in_result(file_, line)
