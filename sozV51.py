...
class LyricsSearcherApp:
    ...
    def filter_file_list(self, event=None):
        raw_query = self.file_search_entry.get().strip()
        if not raw_query:
            # Eğer boşsa tüm dosyaları göster
            self.file_listbox.delete(0, tk.END)
            for filename in sorted(self.file_contents.keys(), key=lambda x: self.normalize(x)):
                self.file_listbox.insert(tk.END, filename)
            return

        keywords = [self.normalize(w) for w in raw_query.split()]
        raw_query_norm = self.normalize(raw_query)
        ...
        for filename in self.file_contents.keys():
            ...
            if raw_query_norm in norm_filename:
                full_match_files.append(filename)
                continue

            match_count = sum(k in norm_filename for k in keywords)
            if match_count > 0 and match_count in grouped_files:
                grouped_files[match_count].append(filename)

        self.file_listbox.delete(0, tk.END)

        # Önce tam eşleşmeler
        for f in sorted(full_match_files, key=lambda x: self.normalize(x)):
            self.file_listbox.insert(tk.END, f)

        # Sonra kısmi eşleşmeler
        for key in sorted(grouped_files.keys(), reverse=True):
            for f in sorted(grouped_files[key], key=lambda x: self.normalize(x)):
                # tam eşleşmeler listesinde olmayanları ekle
                if f not in full_match_files:
                    self.file_listbox.insert(tk.END, f)
