...
class LyricsSearcherApp:
    ...
    def setup_ui(self):
        ...
        center_frame_bottom.grid_rowconfigure(0, weight=1)
        center_frame_bottom.grid_columnconfigure(0, weight=1)

        fav_label = tk.Label(center_frame_bottom, text="Favoriler:", font=self.font_listbox)
    ...
    # ... (Buraya diğer metotlar aynen kalacak, sadece favori liste kaydet/yükle metodları eklenecek)
    ...
    def load_favorites(self):
        if os.path.exists(FAVORITES_FILE):
            try:
                ...
                self.favorites = [f for f in favs if isinstance(f, dict) and
                                  "file" in f and "line" in f and "text" in f]
            except Exception:
                self.favorites = []
        else:
            self.favorites = []
