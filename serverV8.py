...
class TkinterHTTPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        current_files = [
            ...
            if os.path.isfile(os.path.join(path, f)) and f.lower().endswith((".jpg", ".jpeg", ".png", ".gif"))
        ]

        # En yeni dosya en başta olacak şekilde sırala
        current_files.sort(key=lambda a: os.path.getmtime(os.path.join(path, a)), reverse=True)

        # Thumbnail klasöründeki eski dosyaları temizle
        ...
        html = [
            ...
            ".item { margin:20px auto; background:white; padding:10px; border-radius:8px; box-shadow:0 0 5px #ccc; max-width:90%; }",
            ...
            if not os.path.exists(thumb_path):
                try:
                    with Image.open(full_path) as img:
                        img.thumbnail((800, 800))  # Büyük thumbnail
