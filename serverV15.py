...
class TkinterHTTPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def list_videolar(self, path):
        ...
        html = [
            ...
            ".item { margin:16px auto; background:white; padding:12px; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,0.08); max-width: 95%; }",
            ".name { font-size:14px; color:#374151; margin-top:8px; word-break: break-all; }",
            "img, video {display: block; width: 100vw; height: auto; margin: 0; border-radius: 0;}",
        ...
        if dir_list:
            ...
            for d in sorted(dir_list):
                html.append(f"<div class='item'><div class='name'><h3><a href='{d}/'>{d}</a></div></div>")
            html.append("</div>")
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Wi-Fi Medya Paylaşımı V12.0 - Erdal GÜNDOĞAN 2025")
