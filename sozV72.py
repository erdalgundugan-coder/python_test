...
class LyricsSearcherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Kamp Keyfi Şarkı Sözü Arama   -D:\\sozler ve videolar\\sozler-      version V72.0 Erdal GÜNDOĞAN-202510081441")
    ...
    def setup_ui(self):
        ...
        tk.Button(search_frame, text=" ARA\n(özellik:aynı satırdaki kelime,-leri\nkelimelerin ilk heceleri baz alır )", command=self.search_in_all_files).pack(side="left", padx=(0, 1))


        self.btn_new_txt = tk.Button(search_frame, text="DÜZENLE / YENİDEN YAZ \nşarkı sözü metni \nFARKLI KAYDET", command=self.new_txt_toggle)
        self.btn_new_txt.pack(side="left",ipadx=1, ipady=1, padx=(10, 10))
        tk.Button(search_frame, text="DOSYA ÜZERİNE KAYDET__HATA OLMAMASI İÇİN: \nŞarkı söz metni üzende işlem yap seç kopyala\nSoldaki şarkı İSMİ.TXT ye tıkla_altı çizili olsun\nMetni"
        "sil düzenlediğin içeriği yağıştır\nDEĞİŞİK_liği DOSYA ÜZERİNE KAYDET",command=self.emin_misin_save_file).pack(side="left",ipadx=1, ipady=1,fill="x")
        ...
        self.clean_spaces_button.grid(row=2, column=10, pady=5, padx=5, sticky="e")
        save_jpeg_button = tk.Button(right_frame, text="Seçili Metni JPEG Kaydet\n(CTRL+g)(CTRL+j)", command=self.save_selected_text_as_jpeg)
        save_jpeg_button.grid(row=3, column=10, pady=5, padx=5, sticky="e")
        ...
        self.cursor_line_label.grid(row=2, column=0, sticky="w", pady=2)
        ...
        self.dosya_entry.grid(row=2, column=0, sticky="e")
    ...
    def add_selected_result_to_favorites(self):
        ...
        if (filename ==""):
            messagebox.showwarning("Uyarı","Seçili dosya yok...\n.. Arama sonucundan bir dosya.txt seçiniz.")
            return
        yanit2 = messagebox.askyesnocancel("Çıkış", f"....{filename}....\n \n ..Arama sonucundan dosya.txt dosya seçili.\nFavori listesine eklemek ister misiniz?")
        if yanit2 is None:
    ...
    def add_current_line_to_favorites(self):
        ...
        if (filename ==""):
            messagebox.showwarning("Uyarı","seçili dosya yok...\n..soldan dosya.txt seçiniz.")
            return
        yanit2 = messagebox.askyesnocancel("Çıkış", f"....{filename}....\n \n ..soldan dosya.txt dosya seçili.\nFavori listesine eklemek ister misiniz?")
        if yanit2 is None:
            return  # Cancel,  çıkış, iptal tuşuna basıldı,
        elif yanit2:   
        #evet tuşuna basıldı,     
