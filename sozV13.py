...
class LyricsSearcherApp:
    ...
    def setup_ui(self):
        ...
        # Ortadaki: Arama sonuçları ve favoriler
        center_frame.grid_rowconfigure(1, weight=1)
        center_frame.grid_columnconfigure(0, weight=1)

        result_label = tk.Label(center_frame, text="Arama Sonuçları:", font=self.font_listbox)
        result_label.grid(row=0, column=0, sticky="w")

        self.result_listbox = tk.Listbox(center_frame, font=self.font_listbox, exportselection=False)
        self.result_listbox.grid(row=1, column=0, sticky="nsew")
        scrollbar_results = tk.Scrollbar(center_frame, command=self.result_listbox.yview)
        ...
        fav_label = tk.Label(center_frame, text="Favoriler:", font=self.font_listbox)
        fav_label.grid(row=3, column=0, sticky="w")

        self.fav_listbox = tk.Listbox(center_frame, font=self.font_listbox, exportselection=False)
        self.fav_listbox.grid(row=4, column=0, sticky="nsew")
        scrollbar_fav = tk.Scrollbar(center_frame, command=self.fav_listbox.yview)
        scrollbar_fav.grid(row=4, column=1, sticky="ns")
    ...
    def add_selected_result_to_favorites(self):
        ...
        if not sel:
            #messagebox.showwarning("Uyarı", "Arama sonuçlarından bir öğe seçin.")
            return
        idx = sel[0]
        if idx >= len(self.search_results):
            #messagebox.showwarning("Uyarı", "Geçersiz seçim. Lütfen arama yapıp sonuçlardan seçim yapın.")
            return

        filename, lineno, line = self.search_results[idx]
        fav_item = {"file": filename, "line": lineno, "text": line}

        if fav_item in self.favorites:
            #messagebox.showinfo("Bilgi", "Bu kayıt zaten favorilerde.")
            return

        self.favorites.append(fav_item)
        self.save_favorites()
        self.update_fav_list()
        #messagebox.showinfo("Favori", "Arama sonucu favorilere eklendi.")

    def remove_selected_favorite(self):
        sel = self.fav_listbox.curselection()
        if not sel:
            #messagebox.showwarning("Uyarı", "Favorilerden bir öğe seçin.")
            return
        idx = sel[0]
        if idx >= len(self.favorites):
            #messagebox.showwarning("Uyarı", "Geçersiz seçim.")
            return
        removed = self.favorites.pop(idx)
        self.save_favorites()
        self.update_fav_list()
        #messagebox.showinfo("Favori", f"Favoriden çıkarıldı:\n{removed['text']}")
    ...
    def show_favorite_content(self, event=None):
        ...
        try:
            idx_file = self.file_listbox.get(0, tk.END).index(fav["file"])
            self.file_listbox.selection_clear(0, tk.END)
        ...
        self.highlight_text_in_result(fav["file"], fav["line"])
