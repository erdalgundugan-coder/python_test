...
class LyricsSearcherApp:
    ...
    def setup_ui(self):
        ...
        tk.Button(search_frame, text=" ARA\n(özellik:aynı satırdaki kelime,-leri\nkelime başlarındaki heceleri baz alır )", command=self.search_in_all_files).pack(side="left", padx=(0, 10))
        tk.Button(search_frame, text="Favoriden şarkı söz metnini bul\nSoldaki şarkı ismine tıkla-seç mavi işaretli olmalı\nŞarkı söz metni üzende\n DEĞİŞİK_liğini yap ve KAYDET", command=self.save_file).pack(side="left")
        self.btn_new_txt = tk.Button(search_frame, text="Şarkı söz metnini sil/yeni/düzenle\nmetnin ilk satırı başlık olur\nYENİ şarkı sözü metnini KAYDET", command=self.new_txt_toggle)
