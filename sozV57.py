...
class LyricsSearcherApp:
    ...
    def search_in_all_files(self):
        ...
        phrase = self.normalize(raw_input)          # normalize kullanıcı girişi
        search_phrase = phrase.replace(" ", "")    # boşluksuz tam öbek arama
        keywords = phrase.split()                   # kelime kelime arama
        ...
        for filename in search_order:
            ...
            for lineno, line in enumerate(lines):
                if lineno >= y:
                    ...
                    # 🔹 1. Tam öbek kontrolü (boşlukları esnek)
                    line_nospace = norm_line.replace(" ", "")
                    if search_phrase in line_nospace:
                        key = (filename, lineno)
                        if key not in seen:
                            seen.add(key)
                            exact_matches.append((filename, lineno, line.strip()))
                            y = lineno + 40
                        continue

                    # 🔹 2. Kelime başı eşleşme
                    if all(any(w.startswith(k) for w in line_words) for k in keywords):
                        key = (filename, lineno)
                        if key not in seen:
                            seen.add(key)
                            partial_matches.append((filename, lineno, line.strip()))
                            y = lineno + 40

        # 🔹 Önce tam öbek, sonra kısmi eşleşmeler
        final_results = exact_matches + partial_matches

        for idx, (filename, lineno, line) in enumerate(final_results):
            ...
            # 🔹 Listbox renkleri
            if (filename, lineno, line) in exact_matches:
                self.result_listbox.itemconfig(idx, {'bg': 'yellow'})   # tam öbek = sarı
            else:
                self.result_listbox.itemconfig(idx, {'bg': '#add8e6'})  # kısmi eşleşme = mavi
            if is_fav:
                self.result_listbox.itemconfig(idx, {'fg': 'blue'})     # favori = mavi yazı
        ...
        if self.search_results:
            self.highlight_phrase(phrase)               # tam öbek = sarı
            self.highlight_keywords(keywords, phrase)   # kelime bazlı = mavi
...
#.30.08.2025 hazırlan dı aramalr iyileşti,kayıt boyutu ayarlandı