...
class LyricsSearcherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Kamp Keyfi Şarkı Sözü Arama   -D:\\sozler ve videolar\\sozler-      version V75.0 Erdal GÜNDOĞAN-202510081441")
    ...
    def setup_ui(self):
        ...
        tk.Button(search_frame, text=" ...ARA...\nÖrn:(EYLÜ-l de G-el) EYLÜ G yazmalısın.\nÖzellik:Kelime,-lerin İlk Hecelerinden Başlar", command=self.search_in_all_files).pack(side="left", padx=(0, 1))
        ...
        tk.Button(search_frame, text="...KAYDET...\nŞarkı Sözü Metni Üzerinde Değişiklikler Yap\nSoldaki Şarkı Dosyasına Tıklanmış ve Altı_çizili Olsun",command=self.emin_misin_save_file).pack(side="left",ipadx=1, ipady=1,fill="x")
        ...
        copy_btn = tk.Button(right_frame, text="→ Secili-Seçimsiz Metnin\nİlk Satırını/Başlığı Silerek Dörtlük\n Yap(CTRL+f)", command=self.dortluk_yap_ilk_sat_sil)
        copy_btn.grid(row=3, column=2, pady=5, padx=5, sticky="nw")
        copy_btn = tk.Button(right_frame, text="→ Secili-Seçimsiz Metni\nDörtlük Yap\n(CTRL+d)", command=self.dortluk_yap)
        copy_btn.grid(row=3, column=3, pady=5, padx=5, sticky="nw")
        save_jpeg_button = tk.Button(right_frame, text="Seçili-Seçimsiz Metni\nJPEG Gönder\n(CTRL+g)", command= self.save_noselected_text_as_jpeg)#(ctrl+j)
    ...
    def save_selected_text_as_jpeg(self, event=None):
        ...
        save_dir = "D:\\sozler ve videolar\\sozler"
    ...
    def save_noselected_text_as_jpeg(self, event=None):    
        from datetime import datetime
        from PIL import Image, ImageDraw, ImageFont
        self.fazla_bosluklari_temizle()#74 icin eklendi-1111
        save_dir = r"D:\sozler ve videolar\sozler"
        os.makedirs(save_dir, exist_ok=True)
