...
class TkinterHTTPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        html = [
            ...
            #"img, video { width:100%; height:auto; display:block; border-radius:10px; max-width: 90vw; max-height: 80vh; object-fit: contain; }",
            #"img, video {display: block;margin: 0 auto; max-width: 100vw;   /* Görsel her zaman yatayda ekrana sığsın */ max-height: 90vh;   /* Yükseklik ekranı taşmasın */ width: auto;  height: auto; object-fit: contain; border-radius: 10px;}"
            "img, video {display: block; width: 100vw; "
            "  /* Görsel her zaman yatayda ekrana sığsın */"          
            " height: auto; margin: 0; border-radius: 0;}"
            
            "a { color:#2563eb; text-decoration:none; } a:hover { text-decoration:underline; }",
            ".section { margin-top:24px; }",
            "</style></head><body><div class='wrap'>",
            f"<h2><a href='/'>📂 Kamp Keyfi Ana Sayfa</a></h2>",
            f"<h1>📁 {folder_name}</h1>"
        ]
        ...
        if image_files:
            html.append("<div class='section'>")
            html.append("<h3 style='cursor:pointer;' onclick=\"toggleDisplay('images_div')\">🖼️ Kamp Keyfi Resimler ▼</h3>")
        ...
        if video_files:
            html.append("<div class='section'>")
            html.append("<h3 style='cursor:pointer;' onclick=\"toggleDisplay('videos_div')\">🎬 Kamp Keyfi Videolar ▼</h3>")
        ...
        if doc_files:
            ...
            for name in doc_files:
                html.append(f"<div class='item'><div class='name'><a href='{name}' target='_blank'>{name}</a></div></div>")
...
// aynı anda sadece bir video oynasın
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Wi-Fi Medya Paylaşımı V11.0 - Erdal GÜNDOĞAN 2025")
