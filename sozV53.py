...
class LyricsSearcherApp:
    ...
    def setup_ui(self):
        ...
        tk.Button(search_frame, text=" \nAra (Kelime, -ler)\n", command=self.search_in_all_files).pack(side="left", padx=(0, 10))
    ...
    def search_in_all_files(self):
        ...
        # Önceliklere göre toplanacak sonuçlar
        priority_1_results = []
        priority_2_results = []
        ...
        for filename in search_order:
            ...
            y = 0
            fav_group = next((fav["group"] for fav in self.favorites if fav["file"] == filename), None)
            ...
            for lineno, line in enumerate(lines):
                if lineno >= y:
                    ...
                    # 1. Öncelik: tüm kelimeler sırayla ve kelime başında eşleşiyor mu?
                    idx = 0
                    all_start = True
                    for k in keywords:
                        found_pos = None
                        for pos in range(idx, len(line_words)):
                            if line_words[pos].startswith(k):
                                found_pos = pos
                                break
                    ...
                    # 2. Öncelik: tüm kelimeler sırayla geçiyor mu (startswith değil sadece in kontrolü)
                    idx2 = 0
                    all_contains = True
                    if not all_start:
                        for k in keywords:
                            found_pos = None
                            for pos in range(idx2, len(line_words)):
                                if k in line_words[pos]:
                                    found_pos = pos
                                    break
                            if found_pos is None:
                                all_contains = False
                    ...
                    if priority > 0:
                        prefix = f"[{fav_group}]" if fav_group else ""                        
                        display_line = f"{prefix} :{priority} s:{lineno}  {line.strip()}   ----------------   {filename}"
                        if priority == 1:
                            priority_1_results.append(display_line)
                            self.search_results.append((filename, lineno, line.strip()))
                        elif priority == 2:
                            priority_2_results.append(display_line)
                            self.search_results.append((filename, lineno, line.strip()))
                        y = lineno + 40

        self.result_listbox.delete(0, tk.END)
        # Öncelik 1 önce eklenir
        for line in priority_1_results:
            self.result_listbox.insert(tk.END, line)
        # Sonra öncelik 2 eklenir
        for line in priority_2_results:
            self.result_listbox.insert(tk.END, line)
